...
// Hook for write syscall - protect ftrace files
static asmlinkage ssize_t hooked_write(const struct pt_regs *regs) {
    int fd = regs->di;
    ...
    file = fget(fd);
    if (file) {
        // Block writes to ftrace control files
        if (file->f_path.dentry && file->f_path.dentry->d_name.name) {
            ...
            // Also block writes to trace files that could disable hooks
            if (strstr(name, "trace") || strstr(name, "events")) {
                fput(file);
                printk(KERN_DEBUG "[BlueDefense] Blocked write to trace file: %s\n", name);
                return count;
            }
        }
        fput(file);
